/* ALTrack — Aural Lab student session + progress sync to the shared Supabase
   LMS (same project as Music Fundamentals and Piano Lab; course id
   "aural-lab"). Talks to the DB only through SECURITY DEFINER RPCs with the
   publishable key.

   What is recorded: a "practice" event each time the student FINISHES a
   lesson run (Practice or Test mode) — record_lbd(item, activityId). The
   server counts these and marks the item done the first time. Learn-mode
   listening and page visits are never sent.

   Offline: events queue in localStorage with client UUIDs (server dedupes on
   them) and original timestamps; "saved" shows only after confirmation. */
const ALTrack=(()=>{
  const COURSE="aural-lab";
  ...
  const LSS="al-lms-session", LSQ="al-lms-queue";
  ...
  /* ---------- status chip (lesson screens, signed-in students only) ---------- */
  let chip=null;
  function paint(){
    ...
    if(!chip){
      ...
      chip.onclick=()=>{ location.href=(location.pathname.includes("/lessons/")?"../":"")+"student.html"; };
  ...
  /* ---------- academic events ---------- */
  /* one finished lesson run (item = curriculum item_id, activity = mode) */
...
if(typeof module!=="undefined") module.exports=ALTrack;
